...
<div class="LayoutContainer">
    <div class="Layout">
        <label>Name</label>
        <div class="subLayout">
            <span class="gender">{{ $item->gender }} </span> <span class="fullname" style="margin:0 5px;">{{
                $item->fullname }}</span>
        </div>
    </div>
</div>
...
<div class="table2">
    <div class="t2row1">
        <div>
            ...
            <div class="textCustomizing mt-1"><span class="surchargepayment">{{ $item->surchargepayment }}</span><span
                    class="surchargepaymentValue mx-1">{{ $item->surchargePaymentUnit }}</span></div>
        </div>
    </div>
    <div class="t2row2">
        <div class="DHIDisplay" {!! $item->DHI=='0'?'style="display:none;"':'style="display:block;border-top:1px solid
            #000;padding:8px 0px;"' !!}>
            <div class="textCustomizing">In case DHI technique will be applied, there is supplement of <span
                    class="DHI">{{ $item->DHI }}</span><span class="DHIValue mx-1">{{ $item->DHIUnit }}</span></div>
        </div>
    </div>
    {{-- <div class="t2row3">
        <div>
            <div class="textCustomizing">5 % surcharge for payment by credit card:</div>
            <div class="textCustomizing mt-1"><span class="surchargepayment2">{{ $item->surchargepayment2 }}</span><span
                    class="surchargepaymentValue2 mx-1">{{ $item->surchargePaymentUnit2 }}</span>
            </div>
        </div>
    </div> --}}
</div>
<div class="sectionAmount">
    <div class="amountContainer">
        <div>
            ...
            <div class="value">
                <span class="amount">{{ (intval($item->surchargepayment)*0.05) + intval($item->surchargepayment)  }}</span>
                <span class="amountValue">{{ $item->surchargePaymentUnit }}</span>
            </div>
            <hr />
            <div class="tutar">Toplam / Total</div>
            <div class="value mb-2">
                <span class="amount2">{{ (intval($item->surchargepayment)*0.05) + intval($item->surchargepayment)  }}</span>
